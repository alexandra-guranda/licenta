...
def carrefour_precision_extractor(token):
    store_name = "Carrefour"
    output_dir = "outputs/raw_web"
    os.makedirs(output_dir, exist_ok=True)
    filename = f"{output_dir}/data_carrefour.json"
    ...
    all_products = []
    print(f"📡 Scanare CARREFOUR în curs... (Token: {token})")

    # Carrefour are multe pagini, mergem până la 100 de siguranță
    for page in range(1, 101):
        url = f"https://zgarcit.ro/?providers=Carrefour&page={page}&_rsc={token}"
        ...
        try:
            ...
            # Împărțim fluxul RSC în bucăți per produs
            ...
            for chunk in chunks[1:]:
                ...
                if title_m and new_price_m and img_m:
                    ...
                    all_products.append({
                        ...
                        "image_url": img_url,
                        "local_image_path": f"data/images/carrefour/{re.sub(r'[^a-z0-9]+', '_', name.lower()).strip('_')}.jpg"
                    })
                    page_items += 1

            if page_items == 0: break
            print(f"📦 P{page} | Total Carrefour: {len(all_products)}", end="\r")
            time.sleep(0.4)
    ...
    if all_products:
        ...
        print(f"\n\n✨ SUCCES! Carrefour finalizat cu {len(all_products)} produse.")
    else:
        print("\n💀 Token expirat sau nicio pagină găsită pentru Carrefour.")

if __name__ == "__main__":
    TOKEN = "1srqy"
    carrefour_precision_extractor(TOKEN)