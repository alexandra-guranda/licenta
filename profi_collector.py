...
def profi_precision_extractor(token):
    store_name = "Profi"
    output_dir = "outputs/raw_web"
    os.makedirs(output_dir, exist_ok=True)
    filename = f"{output_dir}/data_profi.json"
    ...
    all_products = []
    print(f"📡 Scanare PROFI în curs... (Token: {token})")

    for page in range(1, 80):
        url = f"https://zgarcit.ro/?providers=Profi&page={page}&_rsc={token}"
            ...
            for chunk in chunks[1:]:
                ...
                if title_m and new_price_m and img_m:
                    ...
                    all_products.append({
                        ...
                        "image_url": img_url,
                        "local_image_path": f"data/images/profi/{re.sub(r'[^a-z0-9]+', '_', name.lower()).strip('_')}.jpg"
                    })
                    page_items += 1

            if page_items == 0: break
            print(f"📦 P{page} | Total Profi: {len(all_products)}", end="\r")
            time.sleep(0.4)
    ...
    if all_products:
        ...
        print(f"\n\n✨ SUCCES! Profi finalizat cu {len(all_products)} produse.")
    else:
        print("\n💀 Token expirat sau nicio pagină găsită pentru Profi.")

if __name__ == "__main__":
    TOKEN = "1srqy"
    profi_precision_extractor(TOKEN)