...
def auchan_precision_extractor(token):
    store_name = "Auchan"
    output_dir = "outputs/raw_web"
    os.makedirs(output_dir, exist_ok=True)
    filename = f"{output_dir}/data_auchan.json"
    ...
    all_products = []
    print(f"📡 Scanare AUCHAN în curs... (Token: {token})")

    # Auchan are un catalog foarte mare, mergem până la 120 pagini
    for page in range(1, 121):
        url = f"https://zgarcit.ro/?providers=Auchan&page={page}&_rsc={token}"
        ...
        try:
            ...
            for chunk in chunks[1:]:
                ...
                if title_m and new_price_m and img_m:
                    # Păstrăm numele brut, îl curățăm la unificarea finală
                    ...
                    all_products.append({
                        ...
                        "image_url": img_url,
                        "local_image_path": f"data/images/auchan/{re.sub(r'[^a-z0-9]+', '_', name.lower()).strip('_')}.jpg"
                    })
                    page_items += 1

            if page_items == 0: break
            print(f"📦 P{page} | Total Auchan: {len(all_products)}", end="\r")
            time.sleep(0.4)
    ...
    if all_products:
        ...
        print(f"\n\n✨ SUCCES! Auchan finalizat cu {len(all_products)} produse.")
    else:
        print("\n💀 Token expirat sau nicio pagină găsită pentru Auchan.")

if __name__ == "__main__":
    TOKEN = "1srqy"
    auchan_precision_extractor(TOKEN)