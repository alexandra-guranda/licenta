...
def lidl_precision_extractor(token):
    store_name = "Lidl"
    output_dir = "outputs/raw_web"
    os.makedirs(output_dir, exist_ok=True)
    filename = f"{output_dir}/data_lidl.json"
    ...
    all_products = []
    print(f"📡 Scanare LIDL în curs... (Token: {token})")

    for page in range(1, 68):
        url = f"https://zgarcit.ro/?providers=Lidl&page={page}&_rsc={token}"
            ...
            for chunk in chunks[1:]:
                ...
                if title_m and new_price_m and img_m:
                    ...
                    all_products.append({
                        ...
                        "image_url": img_url,
                        "local_image_path": f"data/images/lidl/{re.sub(r'[^a-z0-9]+', '_', name.lower()).strip('_')}.jpg"
                    })
                    page_items += 1

            if page_items == 0: break
            print(f"📦 P{page} | Total Lidl: {len(all_products)}", end="\r")
            time.sleep(0.5)
    ...
    if all_products:
        ...
        print(f"\n\n✨ SUCCES! Lidl finalizat cu {len(all_products)} produse.")
    else:
        print("\n💀 Token expirat sau nicio pagină găsită. Verifică în Network Tab!")

if __name__ == "__main__":
    # Noul tău token din link:
    TOKEN = "1srqy"
    lidl_precision_extractor(TOKEN)